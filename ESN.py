...
class ESN:
    ...
    '''
    def __init__(self,number_neurons, proba_connexion,number_input,number_output,spectral_radius,leak_rate):
        '''
    ...
    def update(self,input = []):
        ...
        self.y = np.dot(self.W_out,tab1)                     #We use tanh for the output.
        self.n_iter +=1
...
file.close()
total_len = 3*len(mackey_glass)
mackey_glass = 3*mackey_glass
print(total_len)

def compare_MG(esn,starting_iteration):
    simu = esn.simulation(total_len-starting_iteration,[mackey_glass[i] for i in range(starting_iteration)])
#    print(simu)
...
test= ESN(number_neurons = 400, proba_connexion = 0.1, number_input = 1, number_output = 1, spectral_radius = 0.7,leak_rate = 0.9)
